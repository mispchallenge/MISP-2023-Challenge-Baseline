...
def read_textgrid_from_file(filepath):
    ...
    for tier_idx in range(size):
        if filename in filter_list:
            tiers.append(r12_read_tier_from_lines(tier_lines=lines[tier_start[tier_idx] + 1: tier_end[tier_idx]],filepath=filepath)) #四行一个 
        else:
            # tiers.append(read_tier_from_lines(tier_lines=lines[tier_start[tier_idx] + 1: tier_end[tier_idx]],filepath=filepath))# 五行一个 根据textgrid改
...
def read_tier_from_lines(tier_lines,filepath):
    ...
    # import pdb;pdb.set_trace() #eval = *5 dev = *4
